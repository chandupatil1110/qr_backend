...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W} ${H}" width="${W * 3}" height="${H * 3}">
  ...
  <g clip-path="url(#card)">
    ...
    <!-- "QR 4 EMERGENCY" wordmark — sized to fill ~90% of the sticker
         width. Sized down further to fit the compressed 80px header. -->
    ${textPath('QR 4 EMERGENCY', W / 2, 47, {
      font: FONT_HEADING, size: 40, fill: WHITE, anchor: 'middle', letterSpacing: -1.0,
    })}
    ${textPath('SCAN TO CALL OWNER', W / 2, 70, {
      font: FONT_BODY, size: 15, fill: WHITE, anchor: 'middle', letterSpacing: 2.2,
    })}
